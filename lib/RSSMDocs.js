...
// TODO: enable electronic certs
const ENABLE_ELECTRONIC_CERTS = true
...
function makeNamingForm(info, rssm) {
    return new Promise(async function(resolve, reject) {
        ...
        doc.fontSize(12)
        for(let i=0; i<3; i++) {
            // TODO: confirm electronic certificates
            if (ENABLE_ELECTRONIC_CERTS) {
                tableRow(['Anzahl Aktien:', '\u25A1  elektronische Zertifikate'], doc.y, 1)
            } else {
                tableRow(['Anzahl Aktien:'], doc.y, 1)
            }
        ...
        if(ENABLE_ELECTRONIC_CERTS) {
            doc.text('Elektronische Zertifikate werden nicht in Papierform ausgedruckt sondern nur in elektronischer Form im Aktienregister', mm(25), doc.y, cont)
                .text(' verwaltet. ...' )
        }

        doc.moveDown()
        doc.text('Der/die Unterzeichnete/n nehmen zur Kenntniss, dass der Verwaltungsrat über die Namensaktien ein Aktienbuch führt, ', mm(25), doc.y, cont)
            ...
            .text('als 50% des Aktienkapitals zu Eigentum oder Nutzniessung halten darf', mm(32), doc.y)
            .moveDown()
