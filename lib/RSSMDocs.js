...
function makeNamingForm(info, rssm) {
    return new Promise(async function (resolve, reject) {
        ...
        const exportPath = rssm.configFile.get("documentpath")
...
function makeJournalMutation(info, rssm) {
    return new Promise(async function (resolve, reject) {
        ...
        if (register_admins.signature_1) {
            doc.image(register_admins.signature_1, mm(135), mm(245), { height: mm(20) })
        }
...
function makeCertificates(info, rssm) {
    return new Promise(async function (resolve, reject) {
        ...
        for (let i = 0; i < info.shares.length; i++) {
            ...
            if (register_admins.signature_1) {
                doc.image(register_admins.signature_1, mm(50), mm(265), { height: mm(20) })
            }

            if (register_admins.signature_2) {
                doc.image(register_admins.signature_2, mm(110), mm(265), { height: mm(20) })
            }
...
async function positionAddress(doc, address, rssm, drawMarkers = false) {
    ...
    position.left = rssm.configFile.get("addresspos_left")
    position.left = parseInt(position.left)
    position.top = rssm.configFile.get("addresspos_top")
