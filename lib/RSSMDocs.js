...
function makeAnnualReport(info, rssm) {
    return new Promise(async function(resolve, reject) {
        ...
        const tableRow = function(data=[], header=false) {
            ...
            const colWidths = [mm(25), mm(25), mm(20), mm(50), mm(60)];
            let x=0;
            let y=doc.y;
            ...
            if(header) {
                data = ['Datum', 'Journal Nr', 'Transaktion', 'Name', 'Anzahl'];
                doc.font('Arial Bold');
            } else {
                doc.font('Arial');
            }
        ...
        for (let i in info.transactions) {
            ...
            tableRow([t.transaction_date, t.journal_no, t.transaction_type, t.name, t.stock_change]);
        }
