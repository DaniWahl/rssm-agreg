...
function makeJournalIssueReserved(info, rssm ) {
    ...
    return new Promise(async function(resolve, reject) {
        ...
        doc.fontSize(10);
        doc.text('Buchungs Datum', mm(0), mm(200), formLabel);
        doc.text('Journal Reservierung', mm(0), mm(210), formLabel)
...
function makeAnnualReport(info, rssm) {
    return new Promise(async function(resolve, reject) {
        ...
        const tableRow = function(data=[], header=false) {
            ...
            const colPos = [mm(25), mm(30), mm(25), mm(20), mm(30)];
            let x=0;
            let y=doc.y;
            ...
            if(header) {
                data = ['Datum', 'Journal Nr', 'Anzahl', 'Transaktion', 'Name'];
                doc.font('Arial Bold');
            } else {
                doc.font('Arial');
            }
        ...
        for (let i in info.transactions) {
            ...
            tableRow([t.transaction_date, t.journal_no, Math.abs(t.stock_change), t.transaction_type, t.name]);
        }
