...
function makeSharesLetterElectronic(info, rssm) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        doc.text('Wir freuen uns, dass Sie ', cont)
            ...
            .text('auf Sie registriert. Eine Auflistung der Ihnen zugewiesenen Aktien erhalten Sie im beiliegenden Aktienportfolio.')
            .text('Auf Wunsch stellen wir Ihnen die Aktienzertifikate gerne in Papierform aus oder übertragen die Aktien ', cont)
        ...
        if(rssm.data.a_codes[info.a_code]) {    
            ...
            if(rssm.data.a_codes[info.a_code].shares) {
                ...
                for(let i=0; i<rssm.data.a_codes[info.a_code].shares.length; i++) {
                    ...
                    if(is_new) {
                        tableRow([Helpers.pad0(share_no, 3), `Namenaktie (Stammaktie) ${marker_new}`, '1000.--'], doc.y, 'new')
                    } else {
