 ...
 * @returns {Promise<any>}
 */
function makeJournalMutation(info, rssm = rssmShares) {
    return new Promise(async function(resolve, reject) {

        // get configuration values
        const exportPath = await rssm.getConfig('EXPORT_PATH') || './exports/';
        const ag_register = await rssm.getConfig('AG_REGISTER');
        const ag_register_city = await rssm.getConfig('AG_REGISTER_CITY');

        // generate document
        const path = `${exportPath}${Helpers.dateToDbString()}_${info.new_name}_Journal_${info.journal_no}.pdf`;
        ...
        doc.registerFont('Arial Bold', 'assets/fonts/Arial Bold.ttf');
        doc.registerFont('Arial Italic', 'assets/fonts/Arial Italic.ttf');
        ...
        writeStream.on('finish', function() {
            console.log('stream finished, returning path:', path);
            resolve(path);
        });
 ...
 * @returns {Promise<any>}
 */
function makeJournalTransfer(info, rssm = rssmShares) {
    return new Promise(async function(resolve, reject) {
 ...
 * @returns {Promise<any>}
 */
function makeJournalRepurchase(info, rssm = rssmShares) {
    return new Promise(async function(resolve, reject) {
        ...
        // generate document
        const path = `${exportPath}${Helpers.dateToDbString()}_${info.name}_Journal_${info.journal_no}.pdf`;
        const props = getDocProps();
        props.margins = {top: mm(14), left: 0, right: 0};
        const doc = new PDFDoc(props);
        const writeStream = fs.createWriteStream(path);
        ...
        doc.fontSize(12)
            ...
            .text(Helpers.dateToString(new Date(info.transaction_date)), mm(100), mm(200))
            .text(Helpers.dateToString(), mm(100), mm(260));
 ...
 * @returns {Promise<any>}
 */
function makeJournalSale(info, rssm = rssmShares) {
...
function makeCertificates(info, rssm = rssmShares) {
    ...
    return new Promise(async function(resolve, reject) {
        ...
        // generate document
        const path = `${exportPath}${Helpers.dateToDbString()}_${info.name}_Zertifikate_${info.journal_no}.pdf`;
        const props = getDocProps();
        props.margins = {top: mm(14), left: 0, right: 0};
        const doc = new PDFDoc(props);
        const writeStream = fs.createWriteStream(path);
...
function makeSharesLetter(info, rssm = rssmShares) {
    ...
    return new Promise(async function(resolve, reject) {
        ...
        // generate document
        const path = `${exportPath}${Helpers.dateToDbString()}_${info.name}_Begleitbrief_${info.journal_no}.pdf`;
...
function makeAnnualReport(info, rssm = rssmShares) {
    return new Promise(async function(resolve, reject) {
        ...
        // generate document
        const path = `${exportPath}${Helpers.dateToDbString()}_Jahresbericht.pdf`;
        const props = getDocProps();
        props.margins = {top: mm(14), left: 0, right: 0};
        const doc = new PDFDoc(props);
        const writeStream = fs.createWriteStream(path);
        ...
        doc.font('Arial Bold')
            .fontSize(12)
            .text('Aktienkapital:', mm(25), mm(75))
            .moveDown()
        ...
        // Summary data
        doc.font('Arial')
            .text(`${info.shares_total} Namensaktien, CHF ${info.shares_kapital}.-`, mm(75), mm(75))
            .moveDown()
            .text(info.stock)
            .moveDown()
            .text(info.endDate)
