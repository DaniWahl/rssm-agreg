...
function makeNamingForm(info, rssm) {
    return new Promise(async function (resolve, reject) {
        ...
        const ag_register_address = await rssm.getConfig('AG_REGISTER_ADDRESS');
        const ag_register_pc = await rssm.getConfig('AG_REGISTER_POSTCODE');
        ...
        // generate document
        const docPath = path.join(exportPath, `${Helpers.dateToDbString()}_${info.name}_Benennungsformular_${info.journal_no}.pdf`);
        ...
        // document header
        doc.image(assetPath + 'header_color_1.png', mm(25), mm(14), { width: mm(165) });

        // Address
        doc.font('Arial Bold')
        doc.text('Ausgabe von Aktien und Benennung der Aktionäre', mm(25), mm(55))
            .moveDown(1);
        doc.font('Arial')
            .fontSize(10)
        ...
        doc.text('(1):  Elektronische Aktien werden nur in elektronischer Form im Aktienregister erstellt. Sie erhalten keine ', mm(25), doc.y, cont)
           .text('gedruckten Aktienzertifikate auf Papier.')
        doc.text('Bei mehreren Aktien haben Sie die Möglichkeit mehrere Aktieneigentümer zu benennen. Bitte beachten Sie ', mm(25), doc.y, cont)
           .text('dabei Bestimmung a) weiter unten.')
        
        doc.moveDown()
            .font('Arial')
            .text('Der/die Unterzeichnete/n nehmen zur Kenntniss, dass der Verwaltungsrat über die Namensaktien ein Aktienbuch führt, ', mm(25), doc.y, cont)
            .text('in das die Eigentümer der Aktien mit Namen eingetragen werden. Gemäss Art. 5 der Gesellschatsstatuten dürfen die ', cont)
            .text('Namensaktien nur mit Zustimmung des Verwaltungsrates weiterveräussert werden. Der Verwaltungsrat kann ein Gesuch ', cont)
            ...
            .text('Schulangehörige (Lehrer, Verwaltungsratmitarbeiter und Schüler) oder deren Verwante Aktionäre sein können', mm(32), doc.y)
            .moveDown()
        ...
        doc.moveDown(3)
            ...
            .font('Arial Bold')
            .text(`${ag_register}, ${ag_register_address}, ${ag_register_pc} ${ag_register_city}`)


        // Footer
        doc.image(assetPath + 'footer_color_1.png', mm(25), doc.page.height - mm(20), { width: mm(165) });
...
function makeJournalIssueReserved(info, rssm) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        doc.fontSize(30)
            .text('Austellen Zertifikate', mm(20), mm(35));
...
function makeCertificates(info, rssm) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        for (let i = 0; i < info.shares.length; i++) {
            ...
            const certificate = await rssm.selectOneSql('SELECT * FROM share JOIN certificate USING (certificate_id) WHERE share_no=?', [info.shares[i]])
            ...
            doc.font('Courier')
                .fillColor('#aaaaaa')
                .fontSize(5)
                .text(certificate['hash'], 0, mm(290), center)
...
function makeSharesLetterElectronic(info, rssm) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        // get configuration values
        const exportPath = rssm.config.get('documentpath');
        const ag_signature2 = await rssm.getConfig('AG_SIGNATURE2');
        const ag_register = await rssm.getConfig('AG_REGISTER');
        const ag_register_city = await rssm.getConfig('AG_REGISTER_CITY');

        // generate initials
        const ag_register_names = ag_register.split(' ')
        let ag_register_init = ''
        for (let i = 0; i < ag_register_names.length; i++) {
            ag_register_init += ag_register_names[i].substring(0, 1).toUpperCase()
        }

        const tableRow = function (labels, y, header = false) {
        ...
        doc.font('Arial')
            .fontSize(14)
            .text(info.salutation + ' ' + info.first_name + ' ' + info.name, mm(25), mm(50))
            .text(info.address)
        ...
        doc.text('Wir freuen uns, dass Sie ', cont)
            ...
            .text('Münchenstein gezeichnet haben. Wir haben Ihre Aktien in unserem elektronischen Aktienregister', cont)
            .text('auf Sie registriert. Eine Auflistung der Ihnen zugewiesenen Aktien erhalten Sie in der Beilage.')
            .text('Auf Wunsch stellen wir Ihnen gerne Aktienzertifikate in Papierform aus oder übertragen die Aktien', cont)
        ...
        doc.text(ag_register, cont)
            .text(ag_signature2, mm(95));
        doc.text('Aktienregister', cont)
            .text('Verwaltungsrat', mm(92))
            .moveDown(2);
        ...
        doc.moveDown(10)
            .font('Arial Bold')
            .fontSize(12)
            .text('Liste Ihrer Aktien', mm(15))
            .moveDown()
            .font('Arial')
            .fontSize(11)
            .text(`Am ${Helpers.dateToString()} wurden die unten aufgelisteten Aktien der `, cont)
            .text('AG Schulgebäude Rudolf Steiner Schule Münchenstein ', cont)
            .text('auf den folgenden Aktionär registriert:')
            .moveDown()
        ...
        doc.font('Arial Bold')
        tableRow(['Aktien Nr.', 'Bezeichnung', 'Wert (CHF)'], doc.y, true)
        doc.font('Arial')
        for(let i=0; i<info.shares.length; i++) {
            tableRow([info.shares[i], 'Namenaktie (Stammaktie)', '1000.--'], doc.y)
        }

        let totalShares = info.shares.length

        if(info.shares_old) {
            doc.font('Arial')
            .fontSize(11)
            .moveDown()
            .text('Früher erworbene Aktien:', mm(15))
            .moveDown()

            doc.font('Arial Bold')
            tableRow(['Aktien Nr.', 'Bezeichnung', 'Wert (CHF)'], doc.y, true)
            doc.font('Arial')
            for(let i=0; i<info.shares_old.length; i++) {
                tableRow([info.shares_old[i], 'Namenaktie (Stammaktie)', '1000.--'], doc.y)
            }
        }
...
function makeAnnualReport(info, rssm) {
    return new Promise(async function (resolve, reject) {
        ...
        doc.font('Arial Bold')
            .fontSize(14)
            .text('Bericht aus dem Aktienregister', mm(25), mm(53));
