...
class RSSMShares {
    ...
    async changeShareOwner(shares_no, new_a_code, journal_id) {
        ...
        shares_no.forEach(no => {
            ...
            //2DO: need to sort out the level to be incremented
           // cert.level = ???
            cert.status = 'ok';
            promises.push( this.insertCertificate(cert) );

            // 5. update share owner
            promises.push( this.updateRecord('share', share.share_id, {person_id: buyer.person_id}) );

        })

        //2DO: do we need to initialize again after updating the data ?!
