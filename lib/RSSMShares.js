...
class RSSMShares {
    ...
    async saleReserved(transaction, buyer) {
        ...
        for(let i=0; i<comments.length; i++) {
            await this.logComment(comments[i],  {"journal": journal_id, "person": buyer.person_id})
        }
        for(let i=0; i<old_journals.length; i++) {
            await this.logComment(`Von diesen reservierten Aktien wurden Zertifikate erstellt: ${journal.journal_no}`,  {"journal": old_journals[i].journal_id})
        }
    ...
    async getTransactionList(start_date, end_date) {
        ...
        select 
            ...
            case action
                ...
                when 'Reservation' then 'Reservation für'
                else action
        ...
        where transaction_date between ? and ?
          and action in ('Kauf', 'Verkauf', 'Rückkauf', 'Reservation')
