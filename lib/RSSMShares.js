...
class RSSMShares {
    ...
    async logComment(entity, id, value) {
        return this.auditTrail({
            "type" : "comment",
            "entity" : entity,
            "entity_id" : id,
    ...
    async mutation(updated_person) {
        ...
        try {
            ...
            await this.logComment('journal', journal_id, comment)
            await this.updateRecord('person', person.person_id, updated_person);
            await this.logComment('person', person.person_id, comment)
    ...
    async enterPerson(person) {
        ...
        this.logComment("person", person.person_id, comment)
    ...
    async sale(transaction, buyer) {
        ...
        try {
            ...
            if(transaction.comment) {
                this.logComment("journal", journal_id, transaction.comment)
                this.logComment("person", person.person_id, transaction.comment)
            }
    ...
    async transfer(shares_no, a_code_holder, a_code_reciever, comment) {
        ...
        try {
            ...
            const new_journal = await this.insertRecord("journal", journal);
            const journal_id = new_journal.lastID;
            this.logComment("journal", journal_id, comment)
    ...
    async getRecord(entity, id) {
        const sql = `select * from ${entity} where ${entity}_id=?`
