...
class RSSMShares {
    ...
    async init() {
        ...
        try {
            ...
            this.log.info("RSSMShares.init: successfully connected database " + dbPath)

            // retrieve configuration
            this.data.config = await this.getConfig()

            // check for signature files being present on file system
            const files = ["signaturefile_1", "signaturefile_2"]
            files.forEach((file) => {
                const filePath = this.configFile.get(file)

                if (filePath) {
                    // check file path
                    fs.access(filePath, fs.constants.F_OK, (err) => {
                        if (err) {
                            // ignore file since not present on file systems
                            this.logError(`signature image ${file} not found on ${filePath}! ignoring configuration`)
                            this.configFile.ignore(file)
                        }
                    })
                }
            })
    ...
    connectDatabase(dbPath, dbMode) {
        return new Promise((resolve, reject) => {
            ...
            //note: we cannot use this.log in the error handler function!
