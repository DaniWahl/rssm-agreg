...
class RSSMShares {
    ...
    async updateDatabase() {
        ...
        if (update) {
            ...
            if (update.info.current != currentDBv) {
                console.log(`WARNING: will not update database using ${updateFile}. Current database version does not match!`)
    ...
    async saleReserved(transaction, buyer) {
        ...
        if(original_holder.name == buyer.name && original_holder.first_name == buyer.first_name) {
            ...
            transfer_info = await this.transfer(transaction.shares, original_holder.a_code, owner.a_code, "Transfer der Zertifikate im Rahmen der Ausstellung reservierter Aktien")
