...
class RSSMShares {
    ...
    async sale(shares_no, buyer) {
        ...
        if(buyer.a_code) {
            ...
            // 2DO: create new FAMILY if needed

            // create a a_code
            buyer.a_code = buyer.name.toUpperCase();

            // 2DO: need to make sure the a_code is new!
            // 2DO: better to have a numeric a_code from a db sequence.

            // 2DO: need to remove the buyer.holder

            // create new PERSON
            const person_insert = await this.insertPerson(buyer);

            //2DO: changeOwner will probably not cope with a new inserted person not in the cache (need the person_id)
        ...
        try{
            ...
            for(let no of shares_no) {
                ...
                console.log(`RSSMShares.sale: changed ownership of share ${no} shares to ${person.a_code}`);
            }
    ...
    */
    ...
    async generateACode() {
        ...
        //2DO: generation of a-code needs to be more reliable.

        let seq = await this.getSequence('person');
        let value = seq.seq;
        value++;
        return `A-${value}`;
    }
    ...
    /**
     * generate a new F-Code based on the next family sequence value
     * @returns {Promise<string>}
     */
    async generateFCode() {
        let seq = await this.getSequence('family');
        let value = seq.seq;
        value++;
        return `F-${value}`;
    }
    ...
    insertShare(data) {
     ...
     async setConfig(data) {
        ...
        // store config in object
         this.config[data.param] = data.value;

        // do we need to update or insert?
        const row = await this.selectOneSql('SELECT * FROM config WHERE param=?', [data.param]);
        if(typeof row === 'undefined') {
            // need to insert
            sql = this.makeInsertSql('config', data);
        } else {
            // need to update
