...
class RSSMShares {
    constructor(configFile, log, dbMode = sqlite.OPEN_READWRITE) {
        this.data = {
            shareHolders: [],
            persons: [],
            journal: [],
    ...
    async init() {
        ...
        try {
            ...
            // connect database
            const dbPath = this.configFile.get("dbpath")
            this.db = await this.connectDatabase(dbPath, this.dbMode)

            // retrieve configuration
            this.data.config = await this.getConfig()

            // check presense of signature files on this computer
            // TODO: need to change this to configFile
            if (this.configFile.get("signaturefile_1")) {
                fs.access(this.configFile.get("signaturefile_1"), fs.constants.F_OK, (err) => {
                    this.logError(
                        "signature image file 'signaturefile_1' not found! overriding configuration item to undef"
                    )
                    this.configFile.save("signaturefile_1", undefined)
                })
            }

            if (this.configFile.get("signaturefile_2")) {
                fs.access(this.configFile.get("signaturefile_2").AG_REGISTER_SIGNATURE_2, fs.constants.F_OK, (err) => {
                    this.logError(
                        "signature image file 'signaturefile_2' not found! overriding configuration item to undef"
                    )
                    this.configFile.save("signaturefile_2", undefined)
                })
            }
            ...
            this.log.debug(`RSSMShares.init: ${Object.keys(this.data.a_codes).length} share holders loaded`)
            this.log.debug(`RSSMShares.init: ${Object.keys(this.data.shares).length} shares loaded`)

            this.data.history = await this.getShareHolderHistory()

            this.log.debug(`RSSMShares.init: ${this.data.history.length} history entries loaded`)

            // retrieve journal
            this.data.journal = await this.getJournal()

            this.log.debug(`RSSMShares.init: ${this.data.journal.length} journal entries loaded`)

            this.data.rssmShares = this.data.a_codes[this.data.config.RSSM_A_CODE].shares

            this.log.debug(`RSSMShares.init: ${this.data.rssmShares.length} shares owned by RSSM`)
    ...
    async checkForUpdate() {
        const updatFilePath = __dirname.replace("lib", "db")
        const updateFilePattern = /^db_update_.+\.js$/
        let file = null
        this.log.debug("RSSMShares.checkForUpdate: checking for database update scripts...")
        ...
        if (file) {
            return path.join(updatFilePath, file)
        } else {
            return false
        }
    }
    ...
    async updateDatabase() {
        ...
        if (update) {
            if (update.info.current != currentDBv) {
                ...
                this.log.debug(`  our current version: ${currentDBv}`)
                this.log.debug(`  requested version for update: ${update.info.current}`)
                ...
                this.log.info(`RSSMShares.updateDatabase: updating database with ${updateFile}`)
                this.log.info(`  updating database from ${update.info.current} to ${update.info.target}`)

                try {
                    await update.run(this.db)
                } catch (error) {
                    this.log.error("error updating database:")
                    this.log.error(error)
                    errors.push(error)
                }
            }
        } else {
            this.log.error(`update file ${updateFile} can not be read!`)
        }

        if (errors.length) {
            this.log.error("errors occured during database update. please restore the resent database backup")
        }
    }
    ...
    connectDatabase(dbPath, dbMode) {
        return new Promise((resolve, reject) => {
            ...
            this.log.debug(`RSSMShares.connectDatabase: connecting sqlite database '${dbPath}' in mode ${dbMode} ...`)
    ...
    async logActivity(msg, entity = null, id = null) {
        this.log.info("RSSMShares.logActivity", msg)
        return this.auditTrail({
            type: "activity",
            value: msg,
            entity: entity,
    ...
    async auditTrail(data) {
        data["user"] = this.configFile.userInfo.username
    ...
    async logComment(value, entities = {}) {
        const commentData = {
            user: this.configFile.userInfo.username,
            remark: value,
        }
    ...
    async exportToExcel() {
        const maxExports = 5
        const dbpath = this.configFile.get("dbpath")
        const exportPath = this.configFile.get("exportpath")
        const exports = this.configFile.get("exports")

        // make sure a db path is set
        if (!dbpath) {
            this.log.error("  Database not set - can not export backup!")
        }

        // make sure a backup path is set
        if (!exportPath) {
            this.log.error("  Export Path not set - can not create export!")
            return
        }
        ...
        const dbExport = path.join(exportPath, `RSSM_Aktienregister_Export_${helpers.dateToDbString()}.xlsx`)
        await workbook.xlsx.writeFile(dbExport)

        // update configFile
        exports.unshift(dbExport) // add new export to front of array
        this.configFile.save("lastexport", helpers.dateToDbString())
        this.configFile.save("exports", exports.slice(0, maxExports - 1))
    ...
    async backupDatabase(backupNow = false) {
        ...
        const dbpath = this.configFile.get("dbpath")
        const backupPath = this.configFile.get("backuppath")
        const lastBackupStr = this.configFile.get("lastbackup")
        const backups = this.configFile.get("backups")

        // make sure a db path is set
        if (!dbpath) {
            this.log.error("  Database not set - can not create backup!")
        }

        // make sure a backup path is set
        if (!backupPath) {
            this.log.error("  Backup Path not set - can not create backup!")
            return
        }
        ...
        if (backupNow) {
            this.log.info("RSSMShares.backupDatabase: generating database backup...")
            ...
            this.configFile.save("lastbackup", helpers.dateToDbString())
            this.configFile.save("backups", backups.slice(0, maxBackups - 1))
     ...
     * @returns {string}
     */
    async getNextJounalNo() {
        if (!this.initialized) {
            this.log.error("RSSMShares.getNextJournalNo: cannot generate journal number, not initialized!")
            return
        }

        const row = await this.selectOneSql("select max(journal_no) as journal_no from journal")
        const journal_no = row["journal_no"]
        ...
        if (newYear != year) {
            year = newYear
            counter = 0
            this.log.info("RSSMShares.getNextJournalNo: detecting new year - resetting journal counter")
        }
    ...
    async mutation(updated_person) {
        ...
        journal.journal_no = await this.getNextJounalNo()
        ...
        try {
            ...
            return {
                ...
                new_email: updated_person.email,
    ...
    async enterPerson(person) {
        ...
        await this.logComment(comment, { person: person.person_id })
    ...
    async saleReserved(transaction, buyer) {
        ...
        // re-initialize the data cache
        await this.init()
        this.log.debug(`RSSMShares.sale: initialized data objects`)
    ...
    async sale(transaction, buyer) {
        ...
        await this.logActivity(`sold shares '${shares_str}' to person '${person.a_code}'`)
    ...
    async repurchase(shares_no, a_code, booking_date) {
        ...
        try {
            ...
            this.log.debug(`RSSMShares.repurchase: inserted new journal ${journal_id}`)
            ...
            for (let no of shares_no) {
                ...
                updates = updates.concat(share_updates)
                this.log.debug(`RSSMShares.repurchase: changed ownership of share ${no} shares to ${this.rssm_a_code}`)
            }
            ...
            await this.logActivity(`repurchased shares '${shares_str} from person '${person.a_code}'`)

            //2DO: commit db transaction

            // re-initialize the data cache
            await this.init()
            this.log.debug(`RSSMShares.repurchase: initialized data objects`)
    ...
    async transfer(shares_no, a_code_holder, a_code_reciever, comment) {
        ...
        try {
            ...
            for (let no of shares_no) {
                ...
                updates = updates.concat(share_updates)
                this.log.debug(`RSSMShares.transfer: changed ownership of share ${no} shares to ${a_code_reciever}`)
            }
            ...
            await this.logActivity(
                `transferred shares ${shares_str} from person '${holder.a_code}' to person '${reciever.a_code}'`
            )
    ...
    getAdminInfo() {
        return {
            ...
            signature_1: this.configFile.get("signaturefile_1"),
            signature_2: this.configFile.get("signaturefile_2"),
            address_pos_left: this.configFile.get("addresspos_left"),
            address_pos_top: this.configFile.get("addresspos_top"),
        }
    }
    ...
    getJournal() {
        ...
        select 
            ...
            journal_no,
			case 
				when booking_date = '' then journal_date
				when booking_date is null then journal_date
				when booking_date = 'NaN-NaN-NaN' then journal_date
				else booking_date
    ...
    async insertRecord(entity, data) {
        try {
            ...
            this.log.debug(`RSSMShares.insertRecord: created new record ${rec.lastID} in table ${entity}`)
            return rec
        } catch (e) {
            throw new Error(e)
        }
    ...
    async updateRecord(table, id, data) {
        ...
        for (const [field, value] of Object.entries(data)) {
            if (original[field] != value) {
                ...
                this.log.debug(
                    `RSSMShares.updateRecord: updated record ${id} of ${table}.${field} from ${original[field]} to ${value}`
                )
            }
        }
        return rec
    ...
    runSql(sql, params = []) {
        return new Promise((resolve, reject) => {
            this.db.run(sql, params, function (err) {
                if (err) {
                    console.log("ERROR in runSql ", err, sql, params)
                    reject(err)
                } else {
