...
class RSSMShares {
    ...
    constructor(configPath, configSet = 'default', dbMode = sqlite.OPEN_READWRITE) {
        ...
        this.config = new Config(configPath, configSet)
    ...
    async updateDatabase() {
        ...
        if (update) {
            ...
            if (update.info.current != currentDBv) {
                console.log(`ERROR: will not update database using ${updateFile}. Current database version does not match!`)
    ...
    async exportToExcel() {
        ...
        registerSheet.columns = [
            ...
            { header: 'Vorname', key: 'fist_name', width: 25 },
        ...
        journalSheet.columns = [
            ...
            { header: 'Anzahl', key: 'number', width: 10 },
    ...
    async saleReserved(transaction, buyer) {
        ...
        if(original_holder.name == buyer.name && original_holder.first_name == buyer.first_name) {
            ...
            buyer = await this.enterPerson(buyer)

            // TODO: transfer all shares to the new buyer
        ...
        const old_journals = await this.selectAllSql(sql, [buyer.person_id])
        ...
        let comment = `Die Aktienzertifikate wurden von reservierten Aktien erstellt. \n` + 
        `Die Aktienreservierung wurde im Journal unter folgendem Eintrag vermerkt: ${journals_str}`;

        // create journal
        let journal = {};
        journal.journal_date = helpers.dateToDbString();
        journal.journal_no = this.getNextJounalNo();
        journal.a_code = buyer.a_code;
        journal.person_id = buyer.person_id;
        journal.name = buyer.name;
        journal.action = 'Verkauf';
        journal.transaction_type = `Eintrag auf ${buyer.first_name} ${buyer.name}`;;
        ...
        const new_journal = await this.insertRecord("journal", journal);
        const journal_id = new_journal.lastID;
        await this.logComment(comment, {"journal": journal_id})
        if (transaction.comment) {
            await this.logComment(transaction.comment, {"journal": journal_id})
        }
        ...
        const certificates = await this.selectAllSql(sql, [buyer.person_id])
        for (let i=0; i<certificates.length; i++) {
            let c = certificates[i]
            c.status = transaction.cert_type
            c.a_name = buyer.name
            c.a_first_name =  buyer.first_name
            c.journal_id = journal_id
            c.person_id = buyer.person_id
            c = this.hashCertificate(c)
            await this.updateRecord('certificate',c.certificate_id, c)
            await this.logActivity(`activated reserved certificate to ${transaction.cert_type}`, 'certificate', c.certificate_id)
        }
    ...
    async sale(transaction, buyer) {
        ...
        try {
            ...
            const journal_id = new_journal.lastID;
            if(transaction.comment) {
                this.logComment(transaction.comment, {"journal": journal_id, "person": person.person_id})
            }
    ...
    hashCertificate(cert) {
        const hash = CryptoJS.SHA256(`${cert.person_id}${cert.share_id}${cert.journal_id}${cert.a_first_name}${cert.a_name}${cert.transaction_date}`)
    ...
    async changeShareOwner(share_no, new_a_code, journal_id, cert_type=null) {
        ...
        await this.runSql("UPDATE certificate SET status = 'canceled' WHERE certificate_id=?", [old_cert.certificate_id])
    ...
    async getPersonDetail(person_id) {
        ...
        const trail = await this.selectAllSql("select * from audit_trail where entity='person' and entity_id=? order by timestamp desc", [person_id])
    ...
    async setConfig(param, value) {
        ...
        if (typeof row === 'undefined') {
            await this.insertRecord("config", {"param": path, "value": value})
        } else {
