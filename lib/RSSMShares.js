...
class RSSMShares {
    ...
    constructor(dbPath, dbMode=sqlite.OPEN_READWRITE) {
        ...
        console.log(`opening sqlite database ${dbPath} in mode ${dbMode } ...`);
        this.db = new sqlite.Database(dbPath, dbMode, function(err) {
            if(err) {
                throw { name : "Failed to open database " + dbPath, message: err.message};
            } else {
     ...
     * @returns {Promise}
     */
    getTransactionReport(start_date, end_date) {
        let sql = `
        select 
            case booking_date
                when '' then journal_date
                when NULL then journal_date
                else booking_date
