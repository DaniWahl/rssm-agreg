...
class RSSMShares {
    ...
    connectDatabase(dbPath, dbMode) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(typeof dbPath === 'undefined' || dbPath === null) {
                return reject("RSSMShares.connectDatabase: cannot connect to database - database undefined!")
            } else if(!fs.existsSync(dbPath)) {
                return reject(`RSSMShares.connectDatabase: cannot connect to database '${dbPath}' -  file not found!`)
            } 
            ...
            const db = new sqlite.Database(dbPath, dbMode, function(err) {
                if(err) {
                    return reject(`RSSMShares.connectDatabase: Failed to connect database ${dbPath}. (${err.message})`)
                } else {
    ...
    async exportToExcel() {
        ...
        const filename = 'RSSM_Aktienregister_' + helpers.dateToDbString() + '.xlsx'
        const path = config.EXPORT_PATH + '/' + filename
